	...
	for i in range(int(voxel_map_h)):
		...
		for j in range(int(voxel_map_w)):
			...
			if len(valid_pts):
				...
				voxel_m[:len(valid_pts),0] = 1
				voxel_feat[:len(valid_pts),0] = torch.Tensor(valid_pts[:,0]) #x
				voxel_feat[:len(valid_pts),1] = torch.Tensor(valid_pts[:,1]) #y
				voxel_feat[:len(valid_pts),2] = torch.Tensor(valid_pts[:,2]) #z
				voxel_feat[:len(valid_pts),3] = torch.Tensor(valid_pts[:,3]) #r
				voxel_feat[:len(valid_pts),4] = torch.Tensor(valid_pts[:,0] - np.median(valid_pts[:,0])) #x-med
				voxel_feat[:len(valid_pts),5] = torch.Tensor(valid_pts[:,1] - np.median(valid_pts[:,1])) #y-med
				voxel_feat[:len(valid_pts),6] = torch.Tensor(valid_pts[:,2] - np.median(valid_pts[:,2])) #z-med
